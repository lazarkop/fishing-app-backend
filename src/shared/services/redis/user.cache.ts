...
export class UserCache extends BaseCache {
  ...
  public async getRandomUsersFromCache(userId: string, excludedUsername: string): Promise<IUserDocument[]> {
    try {
      ...
      const followees: string[] = await this.client.LRANGE(`following:${userId}`, 0, -1);
      const users: string[] = await this.client.ZRANGE('user', 0, -1);
      const randomUsers: string[] = Helpers.shuffle(users).slice(0, 11);
      for (const key of randomUsers) {
        const followeeIndex = indexOf(followees, key);
        if (followeeIndex < 0) {
