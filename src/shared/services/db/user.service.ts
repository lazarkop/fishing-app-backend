...
class UserService {
  ...
  public async getRandomUsers(userId: string): Promise<IUserDocument[]> {
    ...
    // return users that are followed by logged in user
    const followees: string[] = await followerService.getFolloweesIds(`${userId}`);
    for (const user of users) {
      const followeeIndex = indexOf(followees, user._id.toString());
      if (followeeIndex < 0) {
